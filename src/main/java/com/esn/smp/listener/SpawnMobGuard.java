...
public final class SpawnMobGuard implements Listener {
 ...
 @EventHandler public void chunk(ChunkLoadEvent e){Bukkit.getScheduler().runTask(plugin,()->{for(Entity x:e.getChunk().getEntities())if(mob(x)&&!allowed(x)&&protectedArea(x.getLocation()))x.remove();});}
 private void purgeAll(){for(World w:Bukkit.getWorlds())for(Entity x:w.getEntities())if(mob(x)&&!allowed(x)&&protectedArea(x.getLocation()))x.remove();}
}