...
public final class ESNChatSystem implements Listener, CommandExecutor {
 ...
 private String identity(Player p){int lv=progression==null?1:progression.chatLevel(p);String title=expansion==null?"":expansion.equippedTitle(p);return rank(p)+ChatColor.GOLD+"[Lv. "+lv+"] "+(title==null||title.isBlank()?"":ChatColor.LIGHT_PURPLE+"["+title+"] ")+ChatColor.WHITE+p.getName();}
