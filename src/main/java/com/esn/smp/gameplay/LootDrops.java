...
public final class LootDrops implements Listener{
 ...
 @EventHandler public void death(EntityDeathEvent e){
  ...
  boolean worldBoss=mob.getScoreboardTags().contains("esnWorldBoss"),swampBoss=mob.getScoreboardTags().contains("esnSwampBoss");if(worldBoss||swampBoss){int amount=worldBoss?4:2;ItemStack regular=key(),abyss=abyssKey(),dragon=dragonKey(),eternal=EternalGear.key();regular.setAmount(amount);abyss.setAmount(amount);dragon.setAmount(amount);eternal.setAmount(amount);e.getDrops().add(regular);e.getDrops().add(abyss);e.getDrops().add(dragon);e.getDrops().add(eternal);killer.sendMessage(ChatColor.GOLD+(worldBoss?"WORLD BOSS x4 crate key bundle dropped!":"Boss crate key bundle dropped!"));return;}double eternalChance=(mob instanceof Monster||mob instanceof Slime||mob instanceof Phantom)?0.40:(mob instanceof Animals||mob instanceof WaterMob||mob instanceof Ambient||mob instanceof Villager||mob instanceof WanderingTrader||mob instanceof IronGolem||mob instanceof Snowman)?0.25:0.0;
