...
import org.bukkit.*;import org.bukkit.entity.*;import org.bukkit.event.*;import org.bukkit.event.entity.EntityDeathEvent;import org.bukkit.inventory.*;import org.bukkit.inventory.meta.ItemMeta;import org.bukkit.enchantments.Enchantment;import org.bukkit.persistence.PersistentDataType;import org.bukkit.NamespacedKey;import java.util.concurrent.ThreadLocalRandom;
public final class LootDrops implements Listener{
 private static ItemStack tag(ItemStack i,String id){ItemMeta m=i.getItemMeta();m.addEnchant(Enchantment.SHARPNESS,5,true);m.addItemFlags(ItemFlag.HIDE_ENCHANTS);java.util.List<String> lore=new java.util.ArrayList<>(m.getLore()==null?java.util.List.of():m.getLore());lore.add(ChatColor.LIGHT_PURPLE+"Crate Key V");m.setLore(lore);m.getPersistentDataContainer().set(new NamespacedKey("esnsmp","item_id"),PersistentDataType.STRING,id);i.setItemMeta(m);return i;}
 @EventHandler public void death(EntityDeathEvent e){
  ...
  boolean worldBoss=mob.getScoreboardTags().contains("esnWorldBoss"),swampBoss=mob.getScoreboardTags().contains("esnSwampBoss"),biomeBoss=mob.getScoreboardTags().contains("esnBiomeBoss");if(worldBoss||swampBoss||biomeBoss){if(worldBoss){for(int n=0;n<20;n++){int r=n%4;e.getDrops().add(r==0?key():r==1?abyssKey():r==2?dragonKey():EternalGear.key());}e.getDrops().add(MegaCrates.key(MegaCrates.COUNT-1));killer.sendMessage(ChatColor.GOLD+"WORLD BOSS: exactly 20 core crate keys + 1 guaranteed Realm 100 Key dropped!");}else{ItemStack regular=key(),abyss=abyssKey(),dragon=dragonKey(),eternal=EternalGear.key();regular.setAmount(2);abyss.setAmount(2);dragon.setAmount(2);eternal.setAmount(2);e.getDrops().add(regular);e.getDrops().add(abyss);e.getDrops().add(dragon);e.getDrops().add(eternal);killer.sendMessage(ChatColor.GOLD+"BOSS: exactly 2 of each core crate key dropped!");}return;}double eternalChance=(mob instanceof Monster||mob instanceof Slime||mob instanceof Phantom)?0.40:(mob instanceof Animals||mob instanceof WaterMob||mob instanceof Ambient||mob instanceof Villager||mob instanceof WanderingTrader||mob instanceof IronGolem||mob instanceof Snowman)?0.25:0.0;
