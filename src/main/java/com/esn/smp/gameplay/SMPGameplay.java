...
public final class SMPGameplay implements CommandExecutor, Listener {
 ...
 private ItemStack reward(){int r=new Random().nextInt(100);if(r<5)return new ItemStack(Material.NETHERITE_INGOT);if(r<20)return new ItemStack(Material.DIAMOND,3);if(r<50)return new ItemStack(Material.GOLDEN_APPLE,2);return new ItemStack(Material.EXPERIENCE_BOTTLE,16);}
 ...
 private void leaderboard(Player p){try{p.sendMessage(ChatColor.GOLD+"--- ESN Richest Players ---");int n=1;for(var e:data.topBalances(10))p.sendMessage(ChatColor.YELLOW+""+(n++)+". "+e.playerName()+" - "+e.balance()+" Coins");}catch(Exception x){p.sendMessage(ChatColor.RED+"Leaderboard unavailable.");}}
