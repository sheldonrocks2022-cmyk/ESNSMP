...
import com.esn.smp.gameplay.CastleServices;
...
public final class ESNSMPPlugin extends JavaPlugin {\n    // Build verification marker: combined Castle/NPC/Profile/Mod batch.
 ...
 @Override public void onEnable(){saveDefaultConfig();try{dataStore=new ESNDataStore(getDataFolder(),getConfig().getLong("economy.starting-balance",500),getConfig().getInt("auction.max-listings-per-player",10),getConfig().getLong("auction.max-price",1000000000L));dataStore.initialize();}catch(Exception ex){getLogger().severe("ESN database failed to initialize. Plugin disabled to protect player data: "+ex.getMessage());getServer().getPluginManager().disablePlugin(this);return;}
  ...
  MegaCastle megaCastle=new MegaCastle(this);registerCommand("castlecore",megaCastle);getServer().getPluginManager().registerEvents(megaCastle,this);getServer().getPluginManager().registerEvents(new CastleServices(this),this);JailSystem jail=new JailSystem(this);registerCommand("jail",jail);getServer().getPluginManager().registerEvents(jail,this);
  ...
  getServer().getScheduler().runTask(this,()->{try{spawnManager.ensureConfigured();if(getConfig().getBoolean("spawn.generated",false))hubServices.respawnNpcs();if(getConfig().getBoolean("spawn.build-incomplete",false)){getLogger().severe("Previous spawn build did not finish. Run /esnspawn rollback before rebuilding.");return;}if(getConfig().getBoolean("spawn.build-on-first-start",true)&&!getConfig().getBoolean("spawn.generated",false))spawnManager.buildSpawn(getServer().getConsoleSender());}catch(Exception ex){getLogger().severe("Spawn initialization failed safely: "+ex.getMessage());}});
 ...
 @Override public void onDisable(){if(spawnManager!=null)spawnManager.shutdown();if(discordReminder!=null)discordReminder.shutdown();if(v18Core!=null)try{v18Core.close();}catch(Exception ex){getLogger().severe("v1.8 database close error: "+ex.getMessage());}if(expansionCore!=null)try{expansionCore.close();}catch(Exception ex){getLogger().severe("Expansion database close error: "+ex.getMessage());}if(esn20Core!=null)try{esn20Core.close();}catch(Exception ex){getLogger().severe("ESN 2.0 database close error: "+ex.getMessage());}if(esn20Expansion!=null)try{esn20Expansion.close();}catch(Exception ex){getLogger().severe("ESN 2.0 expansion database close error: "+ex.getMessage());}if(rpgOverhaul!=null)try{rpgOverhaul.close();}catch(Exception ex){getLogger().severe("RPG database close error: "+ex.getMessage());}if(adventureEngine!=null)try{adventureEngine.close();}catch(Exception ex){getLogger().severe("Adventure database close error: "+ex.getMessage());}if(dataStore!=null)try{dataStore.close();}catch(Exception ex){getLogger().severe("Database close error: "+ex.getMessage());}}
 private void registerCommand(String name,org.bukkit.command.CommandExecutor executor){PluginCommand command=Objects.requireNonNull(getCommand(name),"Missing command in plugin.yml: "+name);command.setExecutor(executor);}public SpawnManager getSpawnManager(){return spawnManager;}
}
